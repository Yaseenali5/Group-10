...
{   
    ...
    WTLab = new QLabel(tr("Water Type:"));
    LTLab = new QLabel(tr("Litter Type:"));
...
void LWindow::loadGraph()
{
    ...
    if (!hasData) {
        QLabel *tmp = new QLabel(tr("No litter type found in location/water type\nReconfigure variables"));
        clearWidgets();
    ...
    *scoreSet << scoreCounts[0] << scoreCounts[1] << scoreCounts[2] << scoreCounts[3];
    ...
    chart->setTitle(tr("Scores for Selected Litter Type and Water Type"));
...
void LWindow::checkItemCt()
{
    ...
    if (WTBox->count() == 0) {
        QLabel *msg = new QLabel(tr("Location doesn't have Litters tests\nPlease select another location"));
