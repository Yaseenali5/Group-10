...
{ // this is used to fill in the roll-over pop-ups with appropriate information
  ...
  {
    healthRisks->setToolTip(tr("Polychlorinated biphenyls (PCBs) are a type of POP. Exposure to PCBs can lead to a variety of health problems, including cancer, reproductive problems, immune system problems, and neurological problems."));
  }
  else if (pop.contains("DDT", Qt::CaseInsensitive))
  {
    healthRisks->setToolTip(tr("DDT, a persistent organic pollutant, has been linked to a range of health issues. Long-term exposure can lead to neurological disorders, reproductive problems, and increased cancer risk. Additionally, DDT has detrimental effects on the environment, especially for birds of prey, causing thinning of eggshells and reproductive failure."));
  }
  else if (pop.contains("Aldrin", Qt::CaseInsensitive))
  {
    healthRisks->setToolTip(tr("Aldrin, a potent insecticide and a POP, poses significant health risks. Exposure can lead to neurological disorders, \nincluding tremors and seizures. It can also affect the liver and kidneys. Additionally, aldrin is highly toxic to aquatic life and can disrupt ecosystems."));
  }
}
...
void POPWindow::loadGraph()
{
    ...
    chart->legend()->hide();
    chart->setTitle(QString(tr("Persistent Organic Pollutant Analysis of %1")).arg(selectedLocation));
...
void POPWindow::checkItemCt()
{
    ...
    if (resultBox->count() == 0) {
        QLabel *msg = new QLabel(tr("Location doesn't have POPs\nPlease select another location"));
